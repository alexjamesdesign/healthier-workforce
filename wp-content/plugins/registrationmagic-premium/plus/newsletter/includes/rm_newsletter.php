    ...
    public function subscribe($data,$list_id)
     {
        ...
        if ($status != null) {
            ...
            // Newsletter::STATUS_CONFIRMED replaced with "C" in v5.2.2.0
            if ($status == "C") {
        ...
        if ($user != null) {
            ...
            // Newsletter::STATUS_CONFIRMED replaced with "C" in v5.2.2.0
            if ($user->status == "C" && $opt_in == NewsletterSubscription::OPTIN_DOUBLE) {
        ...
        $user['geo'] = 0;
        // Newsletter::STATUS_CONFIRMED replaced with "C" in v5.2.2.0
        $user['status'] = $opt_in == NewsletterSubscription::OPTIN_SINGLE ? "C" : Newsletter::STATUS_NOT_CONFIRMED;
        ...
        if ($emails) {
            // Newsletter::STATUS_CONFIRMED replaced with "C" in v5.2.2.0
            $newsletter_subscription->send_message(($user->status == "C") ? 'confirmed' : 'confirmation', $user);
        }
