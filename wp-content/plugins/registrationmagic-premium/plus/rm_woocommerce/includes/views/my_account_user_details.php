...
$profile_fields = isset($data->custom_fields) && !empty($data->custom_fields) ? array_keys($data->custom_fields) : array();
?>
...
<div class="rm-submission" id="rm_my_details_tab">
            <div class="rm-user-details-card">
                <div class="rm-user-fields-container1">
                    ...
                    ?>
                    <?php 
                    $exclude_field_type = array('Fname','Lname','Email','Rating','WCBilling','WCShipping');
                    $user_id = get_current_user_id();
                    $user_rm_data = maybe_unserialize(get_user_meta($user_id, 'rm_woo_registration_data', true));
                    if(!empty($user_rm_data)){
                        foreach($user_rm_data as $field_id => $field_data){
                            if(!in_array($field_id,$profile_fields)){
                                if(isset($field_data->type) && !in_array($field_data->type,$exclude_field_type)){
                                    ?>
                                    <div class="rm-user-field-row">
                                        <div class="rm-user-field-label"><?php echo $field_data->label;?></div>
                                        <div class="rm-user-field-value">
                                            <?php
                                            $user_meta_value = $field_data->value;
                                            $meta_field_type = $field_data->type;
                                            if (is_array($user_meta_value) || is_object($user_meta_value)) {
                                                if ($meta_field_type == 'File') {
                                                    foreach ($user_meta_value as $sub) {
                                                        if(absint($sub)){
                                                            $att_path = get_attached_file($sub);
                                                            $att_url = wp_get_attachment_url($sub);
                                                            ?>
                                                ...
                                                } elseif ($meta_field_type == 'Address') {
                                                    $sub = isset($user_meta_value['original']) ? $user_meta_value['original'] : '' . '<br/>';
                                                    if (count($user_meta_value)) {
                                                        $sub .= '<b>Street Address</b> : ' . (isset($user_meta_value['address1']) ? $user_meta_value['address1'] : '') . ', ' . isset($user_meta_value['address2']) ? $user_meta_value['address2']: '' . '<br/>';
                                                        $sub .= '<b>City</b> : ' . isset($user_meta_value['city']) ? $user_meta_value['city'] : '' . '<br/>';
                                                        $sub .= '<b>State</b> : ' . isset($user_meta_value['state']) ? $user_meta_value['state'] : '' . '<br/>';
                                                        $sub .= '<b>Zip code</b> : ' . isset($user_meta_value['zip']) ? $user_meta_value['zip'] : '' . '<br/>';
                                                        $sub .= '<b>Country</b> : ' . isset($user_meta_value['country']) ? $user_meta_value['country'] :'';
                                                    }
                                                    echo $sub;
                                                } elseif ($meta_field_type == 'Time') {                                  
                                                        echo $user_meta_value['time'].", Timezone: ".$user_meta_value['timezone'];
                                                    } elseif ($meta_field_type == 'Checkbox') {   
                                                        echo implode(', ',RM_Utilities::get_lable_for_option($field_id, $user_meta_value));
                                                    } else {
                                                        $sub = implode(', ', $user_meta_value);
                                                        echo $sub;
                                                    }
                                                } else {
                                                if($meta_field_type=='Rating')
                                                {
                                                   echo RM_Utilities::enqueue_external_scripts('script_rm_rating', RM_ADDON_BASE_URL . 'public/js/rating3/jquery.rateit.js');
                                                   $r_sub = array('value' => $user_meta_value,
                                                           'readonly' => 1,
                                                           'max_stars' => 5,
                                                           'star_face' => 'star',
                                                           'star_color' => 'FBC326');
                                                ...
                                                elseif ($meta_field_type == 'Radio' || $meta_field_type == 'Select') {   
                                                    echo RM_Utilities::get_lable_for_option($field_id, $user_meta_value);
                                                }
                                                else{
