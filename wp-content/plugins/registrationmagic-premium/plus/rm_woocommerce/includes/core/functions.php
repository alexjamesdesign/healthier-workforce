...
function rm_wc_show_profile_fields(){ 
    ...
    if ($user instanceof WP_User) {
        ...
        $submissions = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}rm_submissions` WHERE `user_email` = \"{$user->user_email}\"");
        ...
        if (is_array($submissions) || is_object($submissions))
            foreach ($submissions as $submission) {
                if (!in_array($submission, $forms)) {
                    ...
                    $result = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}rm_fields` WHERE `form_id` = $submission->form_id AND `field_show_on_user_page` = 1");
                    if ($result){
                        $sub_data = maybe_unserialize($submission->data);
                        $field_ids[$submission->submission_id] = (object)array('field' => $result, 'sub_data' => $sub_data);
                    }
                }
            }
        ...
        foreach ($field_ids as $submission_id => $res) {
            foreach ($res->field as $f_row) {
                ...
                $result = $wpdb->get_var("SELECT `value` FROM `{$wpdb->prefix}rm_submission_fields` WHERE `submission_id` = $submission_id AND `field_id` = $f_row->field_id");
