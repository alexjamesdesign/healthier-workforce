...
class RM_DBManager_Addon
{
    ...
    public static function update_read_status_all_submissions($form_id, $status)
    {
        ...
        if(empty($form_id))
            $result = $wpdb->query("UPDATE $table_name SET `is_read` = 1");
        else 
   ...
   public static function get_all_form_attachments($form_id,$selection='value')
    {
        ...
        if(empty($form_id)) {
            ...
            $image_ids=  self::get('FIELDS', array('field_type' => 'Image'), array('%s'), 'col', 0, 99999, 'field_id', null, false);
        } else {
    ...
    public static function get_submissions_with_note($form_id){
        ...
        if(empty($form_id)) {
            $qry = "SELECT s.submission_id from $table_submissions s INNER JOIN  $table_notes n on s.submission_id=n.submission_id";
            $submission_ids=$wpdb->get_col($qry);
        } else {
    ...
    public static function get_submissions_by_payment_type($form_id,$type){
        ...
        $table_payment_logs = RM_Table_Tech::get_table_name_for('PAYPAL_LOGS');
        if(empty($form_id)) {
            $qry = "SELECT submission_id from $table_payment_logs where pay_proc in ($type) "; 
            $submission_ids=$wpdb->get_col($qry);
        } else {
     ...
     */
     public static function get_submission_read_count($form_id,$type,$count= true){
        ...
        $where = empty($form_id) ? "" : " and form_id=%d";
        if($count):
            $qry = "SELECT COUNT(*) as count FROM $table_name WHERE is_read=%d{$where}";
            if(empty($where))
                $count= $wpdb->get_var($wpdb->prepare($qry,$type));
            else
                $count= $wpdb->get_var($wpdb->prepare($qry,$type,$form_id));
            return $count;
        else:
            $qry = "SELECT submission_id FROM $table_name WHERE is_read=%d{$where}";
            if(empty($where))
                $submissions_ids= $wpdb->get_col($wpdb->prepare($qry,$type));
            else
    ...
    public static function get_all_submission_read_count($type){
     ...
     public static function get_blocked_submission($form_id){
     	...
     	$table_name = RM_Table_Tech::get_table_name_for('SUBMISSIONS');
     	if(empty($form_id)) {
            $query=  "SELECT submission_id FROM $table_name";
     	    $all_submissions= $wpdb->get_col($query);
        } else {
