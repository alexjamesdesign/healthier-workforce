 ...
 */
namespace NF_FU_VENDOR\phpseclib\Net;

use NF_FU_VENDOR\phpseclib\Crypt\Base;
use NF_FU_VENDOR\phpseclib\Crypt\Blowfish;
use NF_FU_VENDOR\phpseclib\Crypt\Hash;
use NF_FU_VENDOR\phpseclib\Crypt\Random;
use NF_FU_VENDOR\phpseclib\Crypt\RC4;
use NF_FU_VENDOR\phpseclib\Crypt\Rijndael;
use NF_FU_VENDOR\phpseclib\Crypt\RSA;
use NF_FU_VENDOR\phpseclib\Crypt\TripleDES;
use NF_FU_VENDOR\phpseclib\Crypt\Twofish;
use NF_FU_VENDOR\phpseclib\Math\BigInteger;
// Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use NF_FU_VENDOR\phpseclib\System\SSH\Agent;
/**
 * Pure-PHP implementation of SSHv2.
 *
 * @package SSH2
...
class SSH2
{
    ...
    function _generate_identifier()
    {
        ...
        $ext = array();
        if (\function_exists('NF_FU_VENDOR\\Sodium\\library_version_major')) {
            $ext[] = 'libsodium';
        }
    ...
    function _key_exchange($kexinit_payload_server = \false)
    {
        ...
        if (\class_exists('NF_FU_VENDOR\\phpseclib\\Crypt\\RC4') === \false) {
            $encryption_algorithms = \array_diff($encryption_algorithms, array('arcfour256', 'arcfour128', 'arcfour'));
        }
        if (\class_exists('NF_FU_VENDOR\\phpseclib\\Crypt\\Rijndael') === \false) {
        ...
        if (\class_exists('NF_FU_VENDOR\\phpseclib\\Crypt\\Twofish') === \false) {
        ...
        if (\class_exists('NF_FU_VENDOR\\phpseclib\\Crypt\\Blowfish') === \false) {
            $encryption_algorithms = \array_diff($encryption_algorithms, array('blowfish-ctr', 'blowfish-cbc'));
        }
        if (\class_exists('NF_FU_VENDOR\\phpseclib\\Crypt\\TripleDES') === \false) {
        ...
        if ($kex_algorithm === 'curve25519-sha256@libssh.org') {
            ...
            $key = new \NF_FU_VENDOR\phpseclib\Math\BigInteger(\Sodium\crypto_scalarmult($x, $fBytes), 256);
            \Sodium\memzero($x);
        } else {
    ...
    function _get_binary_packet($skip_channel_filter = \false)
    {
        ...
        if ($remaining_length < -$this->decrypt_block_size || $remaining_length > 0x9000 || $remaining_length % $this->decrypt_block_size != 0) {
            if (!$this->bad_key_size_fix && $this->_bad_algorithm_candidate($this->decrypt_algorithm) && !($this->bitmap & \NF_FU_VENDOR\phpseclib\Net\SSH2::MASK_LOGIN)) {
