 ...
 */
use NF_FU_VENDOR\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
...
use NF_FU_VENDOR\Stash\Driver\FileSystem;
use NF_FU_VENDOR\Stash\Pool;
/**
...
class Google_AccessToken_Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('NF_FU_VENDOR\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'NF_FU_VENDOR\\Firebase\\JWT\\JWT';
        }
    ...
    private function getRsaClass()
    {
        if (\class_exists('NF_FU_VENDOR\\phpseclib\\Crypt\\RSA')) {
            return 'NF_FU_VENDOR\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('NF_FU_VENDOR\\phpseclib\\Math\\BigInteger')) {
            return 'NF_FU_VENDOR\\phpseclib\\Math\\BigInteger';
        }
 ...
 *
 */
\class_alias('NF_FU_VENDOR\\Google_AccessToken_Verify', 'Google_AccessToken_Verify', \false);
