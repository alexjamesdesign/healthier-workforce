...
class GCECredentials extends \NF_FU_VENDOR\Google\Auth\CredentialsLoader implements \NF_FU_VENDOR\Google\Auth\SignBlobInterface
{
     ...
     */
    public function __construct(\NF_FU_VENDOR\Google\Auth\Iam $iam = null)
    {
        $this->iam = $iam;
    }
    /**
    ...
    public static function onGce(callable $httpHandler = null)
    {
        ...
        for ($i = 1; $i <= self::MAX_COMPUTE_PING_TRIES; $i++) {
            try {
                ...
                // the metadata resolution was particularly slow. The latter case is
                // "unlikely".
                $resp = $httpHandler(new \NF_FU_VENDOR\GuzzleHttp\Psr7\Request('GET', $checkUri), ['timeout' => self::COMPUTE_PING_CONNECTION_TIMEOUT_S]);
    ...
    {
        $resp = $httpHandler(new \NF_FU_VENDOR\GuzzleHttp\Psr7\Request('GET', $uri, [\NF_FU_VENDOR\Google\Auth\Credentials\GCECredentials::FLAVOR_HEADER => 'Google']));
