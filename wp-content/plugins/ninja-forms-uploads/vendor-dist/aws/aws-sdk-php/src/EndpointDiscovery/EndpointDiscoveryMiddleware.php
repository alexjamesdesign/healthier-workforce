...
namespace NF_FU_VENDOR\Aws\EndpointDiscovery;
...
use NF_FU_VENDOR\Aws\Exception\UnresolvedEndpointException;
use NF_FU_VENDOR\Aws\LruArrayCache;
    ...
    public function __invoke(\NF_FU_VENDOR\Aws\CommandInterface $cmd, \NF_FU_VENDOR\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (isset($op['endpointdiscovery'])) {
            ...
            if ($isRequired || $config->isEnabled()) {
                ...
                $g = function ($value) use($cacheKey, $cmd, $identifiers, $isRequired, $nextHandler, $originalUri, $request, &$endpoint, &$g) {
