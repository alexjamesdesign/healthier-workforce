...
class AssumeRoleWithWebIdentityCredentialProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['client'])) {
            ...
            $this->client = new \NF_FU_VENDOR\Aws\Sts\StsClient(['credentials' => \false, 'region' => 'us-east-1', 'version' => 'latest']);
        }
    }
    /**
    ...
    public function __invoke()
    {
        ...
        return $client->assumeRoleWithWebIdentityAsync($assumeParams)->then(function (\NF_FU_VENDOR\Aws\Result $result) {
            return $this->client->createCredentials($result);
        })->otherwise(function (\RuntimeException $exception) {
            throw new \NF_FU_VENDOR\Aws\Exception\CredentialsException("Error assuming role from web identity credentials", 0, $exception);
        });
    }
}
