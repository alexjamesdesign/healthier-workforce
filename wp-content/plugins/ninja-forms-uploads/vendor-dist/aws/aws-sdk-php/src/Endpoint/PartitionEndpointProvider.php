...
class PartitionEndpointProvider
{
    ...
    public function __construct(array $partitions, $defaultPartition = 'aws')
    {
        $this->partitions = \array_map(function (array $definition) {
            return new \NF_FU_VENDOR\Aws\Endpoint\Partition($definition);
    ...
    {
        $data = \NF_FU_VENDOR\Aws\load_compiled_json(__DIR__ . '/../data/endpoints.json');
        $prefixData = \NF_FU_VENDOR\Aws\load_compiled_json(__DIR__ . '/../data/endpoints_prefix_history.json');
    ...
    public static function mergePrefixData($data, $prefixData)
    {
        ...
        foreach ($data["partitions"] as $index => $partition) {
            foreach ($prefixGroups as $current => $old) {
                $serviceData = \NF_FU_VENDOR\JmesPath\Env::search("services.{$current}", $partition);
                if (!empty($serviceData)) {
                    foreach ($old as $prefix) {
                        if (empty(\NF_FU_VENDOR\JmesPath\Env::search("services.{$prefix}", $partition))) {
