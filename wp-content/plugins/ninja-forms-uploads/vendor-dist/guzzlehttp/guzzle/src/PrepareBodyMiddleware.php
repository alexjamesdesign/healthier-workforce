    ...
    public function __invoke(\NF_FU_VENDOR\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \NF_FU_VENDOR\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\NF_FU_VENDOR\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    private function addExpectHeader(\NF_FU_VENDOR\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
