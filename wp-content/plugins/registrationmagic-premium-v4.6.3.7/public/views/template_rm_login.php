...
{
    echo __('<div class="rmagic"><div class="rmcontent rm-login-wrapper">', 'registrationmagic-gold');
    ...
    if(isset($data->design['placeholder_css'])){		
        $p_css = str_replace("::-", ' #'.$data->otp_form_slug.' ::-', $data->design['placeholder_css']);		
        $p_css = str_replace("}:-", '} #'.$data->otp_form_slug.' ::-', $p_css);	
    ...
    if(!$data->otp_error){
        ...
        if(!empty($data->auth_options['en_resend_otp']) && isset($data->username) && !empty($data->auth_options['otp_resend_limit'])){
            ...
            $form->addElement(new Element_HTML('<div class="rm-resend-otp-liml" onclick="rm_regernate_otp(this,\''.$data->username.'\')"><a href="javascript:void(0)">'.$data->auth_options['otp_resend_text'].'</a></div>'));
        }
    ...
    $form->render();
    echo __('</div></div>', 'registrationmagic-gold');
} 
else // Normal form with username and password
{
    ...
    else 
    {
        ...
        $form->addElement(new Element_HTML('<div class="rmrow"><div class="rmfield"></div><div class="rminput"><ul class="rmradio" style="list-style:none;"><li class="rm-login-remember"> <input id="rm_login_form-element-3-0" type="checkbox" name="remember[]" value="1" checked="checked"><label for="rm_login_form-element-3-0"><span>'.__('Remember Me','registrationmagic-gold').'</span></label> </li> </ul></div></div>'));
