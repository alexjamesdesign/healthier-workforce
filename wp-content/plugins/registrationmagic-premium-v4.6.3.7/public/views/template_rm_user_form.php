...
// Checking if captcha is enabled
          ...
          $check_setting=null;
           ...
           if ($data->is_ccontact_enabled == true && $data->form->form_options->form_is_opt_in_checkbox_cc == 1)
            ...
            $form->addElement(new Element_Button($btn_label != "" ? $btn_label : __('Submit', 'registrationmagic-gold'), "submit", array("bgColor" => "#$data->submit_btn_bgcolor", "fgColor" => "#$data->submit_btn_fgcolor")));
            ...
            else
            {
                ...
                if ($data->expiry_details->state !== 'perpetual')
                {
                    if ($data->expiry_details->state === 'expired')
                        $exp_str .= '<div class="rm-formcard-expired">' . __('Expired','registrationmagic-gold') . '</div>';
                    else
                    {
                        switch ($data->expiry_details->criteria)
                        {
                             case 'both':
                                $exp_str .= '<div class="rm-formcard-expired">' .sprintf(esc_html__('%d out of %d filled and %d days to go','registrationmagic-gold' ),($data->expiry_details->sub_limit - $data->expiry_details->remaining_subs),$data->expiry_details->sub_limit,$data->expiry_details->remaining_days).' </div>';
                            ...
                            case 'subs':
                                ...
                                if ($data->expiry_details->criteria == 'subs')
                                    $exp_str .= '<div class="rm-formcard-expired">' . sprintf(esc_html__('%d out of %d filled.','registrationmagic-gold'),($data->expiry_details->sub_limit - $data->expiry_details->remaining_subs),$data->expiry_details->sub_limit).'</div>';
                                break;

                            case 'date':
                                $exp_str .= '<div class="rm-formcard-expired">' .sprintf(esc_html__('%d days to go.','registrationmagic-gold'),$data->expiry_details->remaining_days). '</div>';
                                break;
                        }
                    }
