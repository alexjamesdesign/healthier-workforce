      ...
      switch ($field->field_type)
      {
            ...
            foreach($vals as $val)
             ...
             foreach($temp as $single_sub)
             {
                ...
                { 
                  ...
                  if(isset($vals[$f_v], $res[$vals[$f_v]]))
                    $res[$vals[$f_v]]+=1;
                  else
        ...
        case 'Select':
            ...
            foreach($temp as $single_sub)
            {
              ...
              if(in_array($single_sub->value, array_keys($vals)))
              {
                 $res[$vals[$single_sub->value]] = (int)$single_sub->count;
                 $tmp_obj->total_sub += (int)$single_sub->count;
              }
        ...
        <script>
            function draw_browser_conversion()
            {
                var data = google.visualization.arrayToDataTable([
                    ["<?php _e('Browser','registrationmagic-gold'); ?>", "<?php _e('Total Visits','registrationmagic-gold'); ?>", "<?php _e('Submissions','registrationmagic-gold'); ?>"],<?php echo $data_string; ?>]);
    ...
    public function sot($form_id,$data=null,$time_range){
        ...
        <script>
            function draw_timewise_stat(){
                var data = google.visualization.arrayToDataTable([
                    ["<?php _e('Date','registrationmagic-gold'); ?>", "<?php _e('Visits','registrationmagic-gold'); ?>", "<?php _e('Submissions','registrationmagic-gold'); ?>"],<?php echo $data_string; ?>]);
