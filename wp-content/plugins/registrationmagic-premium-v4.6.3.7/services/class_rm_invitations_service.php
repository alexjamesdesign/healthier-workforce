...
class RM_Invitations_Service extends RM_Services
{
  ...
  { 
        ...
        $qry = $wpdb->prepare("SELECT * FROM (SELECT * FROM `$sub_table` WHERE `child_id` = 0 AND `form_id` = %d ORDER BY `submission_id` DESC) AS subs GROUP BY subs.user_email LIMIT %d OFFSET %d",$form_id,$limit,$offset);
