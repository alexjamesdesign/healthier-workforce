...
//$this->migrate('paypal_logs', 'crf_paypal_logs', 'rm_paypal_logs');
        ...
        $res = $wpdb->get_results("SELECT * FROM `" . $wpfx . "crf_paypal_log` WHERE 1 LIMIT $offset, $limit", ARRAY_A);
        if (!$res)
            return;
//echo "<pre>", var_dump($res), "</pre>";
...
//Add column if not present.
    ...
    public function crf_migrate_entries_optimized($offset = 0, $limit = 99999999)
    {
        ...
        $src_data = $wpdb->get_results("SELECT * FROM `" . $entry_table . "` WHERE 1 LIMIT $offset,$limit", ARRAY_A);
        $dst_data = array();
