...
abstract class Element extends Base {
	...
	protected $validation = array();
        protected $required_range=null;
	protected $required_max_range=null;
	protected $required_min_range=null;
        protected $advance_opts = array('exclass_row' => '', //This class will be added along with rmrow.
	...
	public function __construct($label, $name, array $properties = null, array $advance_opts = null) {
		$configuration = array(
			...
			"name" => $name
                ...
                if(isset($properties['required_range']))
            $this->required_range=$properties['required_range']; 
                if(isset($properties['required_max_range']))
            $this->required_max_range=$properties['required_max_range']; 
                if(isset($properties['required_min_range']))
            $this->required_min_range=$properties['required_min_range'];
                ...
                if(isset($properties['field_is_unique'])){
                    $properties['data-rm-unique'] =$properties['field_is_unique'];
                    unset($properties['field_is_unique']);
                }
	...
	public function isValid($value,$form_id,$validation_errors=null) {
		$valid = true;
		if(!empty($this->validation)) {
			if(!empty($this->label))
				$element = $this->label;
			...
			foreach($this->validation as $validation) {
				...
				// Handling File validation seperately.
				if($this->getAttribute('type')=='file' && get_class($validation)=="Validation_Required")
                                {
                                    continue;
                                }
				...
				if(!$validation->isValid($value)) {
					...
					if($validation_errors!==null)
                                            $validation_errors->add($this->getAttribute('name'),str_replace("%element%", $element, $validation->getMessage())); 
                                        $valid = false;
