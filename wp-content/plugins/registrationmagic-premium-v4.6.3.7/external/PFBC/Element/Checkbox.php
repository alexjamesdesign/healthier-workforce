...
JS;
	...
	public function render() { 
		...
		$count = 0;
                ...
                $style_str = 'style="list-style:none;"';
                if(isset($this->_attributes["style"]))
                {
                    ...
                    foreach($al as $a)
                    {
                        ...
                        {
                            $style_str ='style="'.$a.';list-style:none;"'; 
                            break;
                        }
                    }
                }
	...
	foreach ($this->options as $value => $text) {
            ...
            if ($sub_ele && $sub_ele instanceof Element) {
                ...
                echo '<li><span class="rm-pricefield-wrap"> <input id="', $this->_attributes["id"], '-', $count, '"', $this->getAttributes(array("id", "value", "checked", "disabled")), ' value="', $this->filter($value), '"';
                ...
                if (isset($this->_attributes["disabled"])) {
                    if (!is_array($this->_attributes["disabled"]) && $this->_attributes["disabled"])
                        echo ' disabled';
                    else if ($value && is_array($this->_attributes["disabled"]) && in_array($value, $this->_attributes["disabled"]))
                        echo ' disabled';
                }
                //echo '/> ', $text, ' </label> ';
                ...
                echo '<li> <input id="', $this->_attributes["id"], '-', $count, '"', $this->getAttributes(array("id", "value", "checked", "disabled")), ' value="', $this->filter($value), '"';
                //echo '<label class="', $labelClass, '"> <input id="', $this->_attributes["id"], '-', $count, '"', $this->getAttributes(array("id", "value", "checked", "required")), ' value="', $this->filter($value), '"';
                if ($value && in_array($value, $this->_attributes["value"]))
                    echo ' checked="checked"';
                if (isset($this->_attributes["disabled"])) {
        ...
        if(isset($this->_attributes["rm_is_other_option"]) && $this->_attributes["rm_is_other_option"] == 1){                    
                   ...
                   if(!$other_val)
                   {
                   echo '<input type="checkbox" value="" id="'.$this->_attributes["id"].'_other" name="'.$this->getAttribute("name").'" style="'.$this->getAttribute("style").'"><label for="'.$this->_attributes["id"].'_other">'.__('Other','registrationmagic-gold').'</label></li>'.
                   ...
                   {
                    echo '<input type="checkbox" value="" id="'.$this->_attributes["id"].'_other" name="'.$this->getAttribute("name").'" style="'.$this->getAttribute("style").'" checked><label for="'.$this->_attributes["id"].'_other">'.__('Other','registrationmagic-gold').'</label></li>'.
