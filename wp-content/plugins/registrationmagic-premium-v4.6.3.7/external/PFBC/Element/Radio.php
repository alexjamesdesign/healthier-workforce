		...
		foreach($this->options as $value => $text) {
			...
			++$count;
                ...
                if(isset($this->_attributes["rm_is_other_option"]) && $this->_attributes["rm_is_other_option"] == 1){                       //get value of "other" field to be prefilled if provided.
                   ...
                   echo '<li>';
                   if($other_val){
                     echo      '<input id="'.$this->_attributes["id"].'_other" type="radio" value="" name="'.$this->getAttribute("name").'" style="'.$this->getAttribute("style").'" checked><label for="'.$this->_attributes["id"].'_other">'.__('Other','registrationmagic-gold').'</label></li>'.
                   ...
                   {
                     echo  '<input id="'.$this->_attributes["id"].'_other" type="radio" value="" name="'.$this->getAttribute("name").'" style="'.$this->getAttribute("style").'"><label for="'.$this->_attributes["id"].'_other">'.__('Other','registrationmagic-gold').'</label></li>'.
