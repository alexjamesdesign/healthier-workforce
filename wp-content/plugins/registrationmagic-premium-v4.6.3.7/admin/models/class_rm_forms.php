...
class RM_Forms extends RM_Base_Model
{
    ...
    public function get_form_pages() {
        $pages = new stdClass;
        if (!$this->form_options->form_pages) {
            $pages->count = 1;
            $pages->data = array('Page 1');
            $pages->order = array(0);
        } else {
            $pages->count = count($this->form_options->form_pages);
            $pages->data = $this->form_options->form_pages;
            if (!$this->form_options->ordered_form_pages)
            {
                $pages->order = array_keys($form_pages);
            }
            else 
                $pages->order = $this->form_options->ordered_form_pages;
        }
    ...
    public function set_sub_limit_ind_user($value){
        $this->form_options->sub_limit_ind_user = $value;
    }
    
    public function set_hide_username($value){
        $this->form_options->hide_username = $value;
    }
    ...
    public function set_form_is_opt_in_checkboxcc($value)
    {
        if(is_array($value) || is_object($value)){
            $this->form_options->form_is_opt_in_checkboxcc = count($value);
        }else{
            $this->form_options->form_is_opt_in_checkboxcc = 0;
        }
    }
    ...
    {
        $valid = true;
        $access_control = $this->form_options->access_control;
    ...
    public function validate_accounts($form_builder_id)
    {
        ...
        {
            if ($this->default_form_user_role == "" && empty($this->form_options->form_should_user_pick) == 0)
            {
