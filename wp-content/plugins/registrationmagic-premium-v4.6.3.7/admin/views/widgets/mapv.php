        ...
        geocoder.geocode({'location': latlng}, function (results, status) {
            if (status === google.maps.GeocoderStatus.OK) {
                if (results[1]) {
                    ...
                    window.alert("<?php _e('No Results Found.', 'registrationmagic-gold'); ?>");
                }
            } else {
                window.alert('<?php _e('Geocoder failed due to:', 'registrationmagic-gold'); ?>' + status);
            }
        });
    }
            ...
            if(address){
             ...
             geocoder.geocode({'address': address}, function (results, status) {
                if (status === google.maps.GeocoderStatus.OK) {
                   ...
                   alert('<?php _e('Geocode was not successful for the following reason: ', 'registrationmagic-gold'); ?>' + status);
                }
            });
