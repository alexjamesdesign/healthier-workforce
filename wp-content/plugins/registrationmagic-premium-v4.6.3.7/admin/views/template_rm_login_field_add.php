...
<?php else: ?>
    <div class="rmagic">
        ...
        <div class="rmcontent">
            ...
            $form->addElement(new Element_HTML('<div class="rmheader">'.__('Edit Field', 'registrationmagic-gold').'</div>'));
            if ($field == 'username') {
                // Username fields
                $form->addElement(new Element_Select("<b>".__('Username field accepts', 'registrationmagic-gold')."</b>", "username_accepts", array('username' => __("Username",'registrationmagic-gold'), 'email' => __("Email",'registrationmagic-gold'), 'ue' => __("Username and Email",'registrationmagic-gold')), array("value" => $data->field['username_accepts'], "class" => "rm_static_field rm_required", "required" => "1", "longDesc" => __('Define what the Username field accepts in your login form. You can allow it to accept both username and email associated with the user\'s account or only one of them.', 'registrationmagic-gold'))));
                $form->addElement(new Element_Textbox("<b>".__('Username field label', 'registrationmagic-gold')."</b>", "field_label", array("class" => "rm_static_field", "value" => $data->field['field_label'], "longDesc" => __('Label of the Username field in Login Form on frontend.', 'registrationmagic-gold'))));
                $form->addElement(new Element_Textbox("<b>".__('Username field placeholder', 'registrationmagic-gold')."</b>", "placeholder", array("class" => "rm_static_field", "value" => !isset($data->field['placeholder']) ? __('Enter Username','registrationmagic-gold') : $data->field['placeholder'], "longDesc" => __('Placeholder is the faded background text users see inside the input box before they start typing in. It is a browser feature and commonly used on sites to guide the user about filling the input field value.', 'registrationmagic-gold'))));
                $form->addElement(new Element_HTML('<div class="rmrow"><div class="rmfield" for="login-add-field-element-3"><label><b>'.__('Username Field Empty Error','registrationmagic-gold').'</b></label></div><div class="rminput"><div class="rmnotice" style="width:372px">'.__("The username empty error message is automatically handled by the user's browser in its default language.",'registrationmagic-gold').'</div></div><div class="rmnote"><div class="rmprenote"></div><div class="rmnotecontent"></div></div></div>'));
            } 
            else if ($field == 'password') {
                // Password fields
                $form->addElement(new Element_Textbox("<b>".__('Password field label', 'registrationmagic-gold')."</b>", "field_label", array("class" => "rm_static_field", "value" => $data->field['field_label'], "longDesc" => __('Label of the Password field in Login Form on frontend.', 'registrationmagic-gold'))));
                $form->addElement(new Element_Textbox("<b>".__('Password field placeholder', 'registrationmagic-gold')."</b>", "placeholder", array("class" => "rm_static_field", "value" => !isset($data->field['placeholder']) ? __('Enter Password','registrationmagic-gold') : $data->field['placeholder'], "longDesc" => __('Placeholder is the faded background text users see inside the input box before they start typing in. It is a browser feature and commonly used on sites to guide the user about filling the input field value.', 'registrationmagic-gold'))));
               
                $form->addElement(new Element_HTML('<div class="rmrow"><div class="rmfield" for="login-add-field-element-3"><label><b>'.__('Password Field Empty Error','registrationmagic-gold').'</b></label></div><div class="rminput"><div class="rmnotice" style="width:372px">'.__("The password empty error message is automatically handled by the user's browser in its default language.",'registrationmagic-gold').'</div></div><div class="rmnote"><div class="rmprenote"></div><div class="rmnotecontent"></div></div></div>'));
            }
            ...
            $form->addElement(new Element_HTMLL('&#8592; &nbsp; '.__('Cancel','registrationmagic-gold'), '?page=rm_login_field_manage', array('class' => 'cancel')));
